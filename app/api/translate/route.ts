...
function isAnalysis(value: unknown): value is Analysis {
  ...
  return typeof analysis.title === "string" && Array.isArray(analysis.summary) && analysis.summary.every((item) => typeof item === "string") && Array.isArray(analysis.keyIdeas) && analysis.keyIdeas.every((item) => typeof item === "string") && typeof analysis.simpleExplanation === "string" && Array.isArray(analysis.flashcards) && analysis.flashcards.every(isFlashcard);
}
function isTranslationDocument(value: unknown): value is TranslationDocument {
  if (!value || typeof value !== "object") {
    return false;
  }
  const document = value as Record<string, unknown>;
  return typeof document.id === "string" && typeof document.fileName === "string" && isAnalysis(document.analysis);
}
function isTranslationResult(value: unknown): value is TranslationResult {
  ...
  return typeof item.id === "string" && isAnalysis(item.analysis);
}
...
function isRetryableError(error: unknown) {
  ...
  return message.includes("429") || message.includes("500") || message.includes("502") || message.includes("503") || message.includes("504") || message.includes("resource exhausted") || message.includes("overloaded") || message.includes("unavailable") || message.includes("timeout");
}
...
// Guards against a repeat of the gemini-flash-lite-latest incident: if the
// configured fast model ID is ever invalid/retired, fall back to the
// known-good reasoning model instead of every translation failing outright.
function isModelNotFound(error: unknown) {
  const message = getErrorMessage(error).toLowerCase();
  return message.includes("not_found") || message.includes("404") || (message.includes("model") && message.includes("not found"));
}
...
function withTimeout<T>(operation: () => Promise<T>, deadline: number): Promise<T> {
  ...
  if (remaining <= 0) {
    return Promise.reject(new Error("Translation is taking too long — please try again in a moment."));
  }
  return new Promise<T>((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      reject(new Error("Translation is taking too long — please try again in a moment."));
    }, remaining);
    operation().then(
      (value) => {
...
function buildSingleDocumentPrompt(document: TranslationDocument, languageName: string) {
  return `
...
async function translateDocument(document: TranslationDocument, languageName: string, apiKey: string, initialModel: string, deadline: number): Promise<TranslationResult> {
  const prompt = buildSingleDocumentPrompt(document, languageName);
    ...
    try {
      ...
      try {
        ...
        if (parsedData && typeof parsedData === "object") {
          data = parsedData as GeminiResponse;
        }
      } catch {
        data = null;
      }
      ...
      if (!isTranslationResult(extracted)) {
      ...
      lastError = error;
      if (isModelNotFound(error) && model !== "gemini-flash-latest") {
...
export async function POST(request: NextRequest) {
  try {
    ...
    await consumeUsage(usageUser.id, "translate");
    const rawBody = (await request.json()) as unknown;
    const body: TranslationRequestBody = rawBody && typeof rawBody === "object" ? (rawBody as TranslationRequestBody) : {};
    ...
    const documents = documentsValue.filter(isTranslationDocument);
    ...
    // User priority is speed over polish for translation — use the
    // fastest available model. Falls back to the reasoning-tier model
    // automatically if this ID is ever retired (see isModelNotFound
    // handling pattern in lib/gemini.ts).
    const model = process.env.GEMINI_MODEL_FAST?.trim() || "gemini-3.5-flash-lite";
    ...
    const settled = await Promise.allSettled(documents.map((document) => translateDocument(document, languageName, apiKey, model, deadline)));
    let quotaExhausted = false;
    const translations: TranslationResult[] = settled.map((result, index) => {
      if (result.status === "fulfilled") {
        return result.value;
      }
      if (isDailyQuotaExhausted(result.reason)) {
        quotaExhausted = true;
      }
      ...
      return {
        ...
        analysis: documents[index].analysis,
      };
    });
    if (quotaExhausted && settled.every((result) => result.status === "rejected")) {
      return NextResponse.json(
        {
          error: "Gemini API quota is exhausted. The translation request was stopped safely. Please wait for the quota reset or use a Gemini API project with available quota.",
